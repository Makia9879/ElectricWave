...
import com.makia98.electricwave.ui.NotificationListScreen
import com.makia98.electricwave.ui.ProfileScreen
import com.makia98.electricwave.ui.theme.ElectricWaveTheme
...
private fun AppNavigation(
    ...
    // DEBUG-only: open a destination directly (adb `am start --es nav list|diagnostics`).
    LaunchedEffect(pendingNav) {
        when (pendingNav) {
            "list" -> { route = "list"; onConsumePendingNav() }
            "diagnostics" -> { route = "diagnostics"; onConsumePendingNav() }
            else -> Unit
        }
    }
    ...
    when (route) {
        ...
        "diagnostics" -> DiagnosticsScreen(
            viewModel = viewModel,
            onBack = { route = "config" },
            onOpenAppNotificationSettings = { openAppNotificationSettings(viewModel) },
            onOpenChannelSettings = { id -> openChannelSettings(viewModel, id) },
        )
        else -> ProfileScreen(
            viewModel = viewModel,
            notificationData = notificationData,
...
private fun openAppNotificationSettings(viewModel: NoticeViewModel) {
...
private fun openChannelSettings(viewModel: NoticeViewModel, channelId: String) {
    val ctx = viewModel.getApplication<android.app.Application>()
