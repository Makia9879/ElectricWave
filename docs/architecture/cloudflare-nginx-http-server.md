...
对外域名固定为 `notice.makia98.com`。`PUBLIC_BASE_URL` 固定为 `https://notice.makia98.com`；Android 生产 profile 只接受该 HTTPS 地址。
...
```dotenv
HTTP_ADDR=:8788
PUBLIC_BASE_URL=https://notice.makia98.com
...
- `notice.makia98.com` 的 A 记录指向 `47.119.174.236`，并保持 **DNS only**（灰云）。
...
- VPS Nginx 通过 Let's Encrypt 的 HTTP-01 challenge 获取公信 TLS 证书。`80/tcp` 与 `443/tcp` 必须允许公网访问；SSH `5022/tcp` 仍应按管理 IP 限制。
- 证书保存于 `/home/mk/docker-compose/nginx/certbot/conf`，Nginx 以只读方式挂载并使用 `fullchain.pem` 与 `privkey.pem`。
- Certbot 使用 webroot `/home/mk/docker-compose/nginx/certbot/www`，Nginx 在 `/.well-known/acme-challenge/` 直接提供该目录，不代理至应用。

网络切换或源站重启导致 SSE 重建是正常故障模型，Android 前台服务按既定指数退避策略重连。30 秒 SSE 注释心跳必须持续发送，以避免 Nginx 和中间代理将空闲连接关闭。

## Nginx 虚拟主机

现有 `nginx` 容器使用 host 网络，并只读挂载 `/home/mk/docker-compose/nginx/conf` 至 `/etc/nginx`。新增独立的 `notice.makia98.com.conf`，不修改已有 `makia98.com` 与 `aiops.makia98.com` 站点。

证书与 ACME webroot 应单独挂载：

```text
/home/mk/docker-compose/nginx/certbot/conf:/etc/letsencrypt:ro
/home/mk/docker-compose/nginx/certbot/www:/var/www/certbot:ro
```
...
server {
    listen 80;
    listen [::]:80;
    server_name notice.makia98.com;
...
server {
    ...
    ssl_certificate     /etc/letsencrypt/live/notice.makia98.com/fullchain.pem;
    ssl_certificate_key /etc/letsencrypt/live/notice.makia98.com/privkey.pem;
...
1. `http://notice.makia98.com/.well-known/acme-challenge/<token>` 可由 Let's Encrypt 直接访问；其他 HTTP 路径跳转到 HTTPS。
